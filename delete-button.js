    ...
    function DeleteButton() {
        ...
        _this._onStatusChange = function (id, oldStatus, newStatus) {
            if (id === _this.props.id && !_this._unmounted) {
                if (!isDeletable(newStatus) && newStatus !== 'deleting' && _this.state.deletable) {
                    !_this._unmounted && _this.setState({
                        deletable: false,
                        deleting: false
                    });
                    _this._unregisterStatusChangeHandler();
                } else if (isDeletable(newStatus) && !_this.state.deletable) {
                    _this.setState({
                        deletable: true,
                        deleting: false
                    });
                } else if (newStatus === 'deleting' && !_this.state.deleting) {
...
DeleteButton.propTypes = {
    ...
    onlyRenderIfDeletable: _react.PropTypes.bool,
...
var isDeletable = function isDeletable(status) {
    return ['delete failed', 'upload successful'].indexOf(status) >= 0;
};
